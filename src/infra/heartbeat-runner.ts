...
import { resolvePlannerDecision } from "../planner/planner-orchestrator.js";
...
After completing all due tasks, reply HEARTBEAT_OK.`;
  ...
  const promptedBase = appendHeartbeatWorkspacePathHint(basePrompt, params.workspaceDir);
  ...
  const plannerRows = await readPlannerRows({
    sessionKey: params.preflight.session.sessionKey,
  });
  const recoveryActions = resolvePlannerRecoveryActions({
    nowMs: params.startedAt,
    rows: plannerRows,
  });
  let prompt = buildHeartbeatAttentionPrompt({
    basePrompt: promptedBase,
    nowMs: params.startedAt,
    entry: params.preflight.session.entry,
    pendingEvents: pendingEventEntries,
  });
  if (plannerDecision.promptBlock) {
    prompt += `

${plannerDecision.promptBlock}`;
  }
  try {
    const memoryContext = await buildMemoryContext();
    if (memoryContext && memoryContext !== "No prior memory available.") {
      prompt += `\n\n${memoryContext}`;
    }
  } catch {
  ...
  if (recoveryActions.length > 0) {
    const lines = [
      "Planner recovery watch:",
      ...recoveryActions
        .slice(0, 3)
        .map((action) => `- ${action.action} ${action.rowId}: ${action.reason}`),
      "- If you reference recovery, keep it concise and low-drama.",
    ];
    prompt += `
...
Reply with approve/reject or edit your version to confirm.`;
  ...
  if ((plannerDecision?.mode === "spawn_readonly" || plannerDecision?.mode === "send") && plannerRow && spawnRateLimited) {
    plannerRow =
      (await updatePlannerRow({
        sessionKey,
        rowId: plannerRow.id,
        ...
        patch: {
          status: "blocked",
          note: "Planner spawn rate limited: max 10 worker spawns per hour per session.",
        },
      })) ?? plannerRow;
    prompt += `

Planner worker:
- spawn blocked by hourly rate limit
- keep the next step surfaced or wait before spawning more workers`;
  }

  if (plannerDecision?.mode === "spawn_readonly" && plannerRow && !spawnRateLimited) {
...
Planner worker:
- spawned read-only worker for this goal
...
- keep the next step low-blast-radius and do not assume a worker is running`;
  ...
  if (plannerDecision?.mode === "send" && plannerRow && !spawnRateLimited) {
    const spawnResult = await spawnPlannerReadonlyTask({
      cfg,
      agentId,
      sessionKey,
...
Planner worker:
- spawned worker to produce draft for send approval
- child session: ${spawnResult.childSessionKey ?? "unknown"}
- draft will be delivered for Dom's confirmation before any send occurs
- do not spawn another worker for the same goal unless this one fails or stalls`;
    } else if (spawnResult.status === "skipped") {
      const status = spawnResult.reason === "existing-active-spawn" ? "dropped" : "blocked";
      plannerRow =
...
Planner worker:
- a worker is already active for this send goal
- do not spawn another worker unless the current one fails or stalls`;
      }
    } else {
      plannerRow =
